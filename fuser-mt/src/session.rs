...
impl<FS: Filesystem> Session<FS> {
    ...
    /// Run the session loop in single-threaded mode until FUSE init completes.
    /// Returns Ok(true) if init succeeded, Ok(false) if the session ended
    /// before init (e.g. unmount), or Err on I/O error.
    fn run_until_initialized(&mut self) -> io::Result<bool> {
        let mut buffer = vec![0; BUFFER_SIZE];
        let buf = aligned_sub_buf(
            buffer.deref_mut(),
            std::mem::align_of::<abi::fuse_in_header>(),
...
impl<FS: 'static + Filesystem + Clone + Send> Session<FS> {
    /// Run the session loop with multiple worker threads for concurrent request
    /// handling.  The FUSE `init` handshake is completed single-threaded on the
    /// calling thread, then `num_workers` additional threads are spawned.  The
    /// calling thread also participates as a worker, so the total concurrency is
    /// `num_workers + 1`.
    ///
    /// Each worker owns its own buffer and an independent clone of `FS`.
    /// Concurrent reads from `/dev/fuse` are safe: the kernel FUSE driver
    /// guarantees that each `read()` returns exactly one complete request.
    pub fn run_multithreaded(&mut self, num_workers: usize) -> io::Result<()> {
        // Phase 1: single-threaded init handshake
        if !self.run_until_initialized()? {
            return Ok(());
        }
        ...
        let handles: Vec<JoinHandle<io::Result<()>>> = (0..num_workers)
            .map(|_| {
                let ch = self.ch.clone_for_worker();
                let mut fs = self.filesystem.clone();
                let sender = self.ch.sender();
                let allowed = self.allowed.clone();
                let session_owner = self.session_owner;
                let proto_major = self.proto_major;
                let proto_minor = self.proto_minor;
        ...
        // Phase 3: main thread also works as a worker
        let main_result = self.run();

        // Signal workers to stop and wait for them
        destroyed.store(true, Ordering::Release);
        for handle in handles {
            match handle.join() {
                Ok(Err(e)) => warn!("FUSE worker thread error: {}", e),
                Err(_) => warn!("FUSE worker thread panicked"),
                Ok(Ok(())) => {}
            }
        }
