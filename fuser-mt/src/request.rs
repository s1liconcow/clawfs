...
impl<'a> Request<'a> {
    ...
    /// Dispatch a request on a worker thread without needing a full `Session`.
    /// This is called for all requests after init has completed.
    /// Init and Destroy are never dispatched to workers.
    pub(crate) fn dispatch_on_worker<FS: Filesystem>(
        &self,
        fs: &mut FS,
        allowed: &SessionACL,
        session_owner: u32,
    ...
    fn dispatch_worker_req<FS: Filesystem>(
        &self,
        se_fs: &mut FS,
        allowed: &SessionACL,
        session_owner: u32,
        ...
        match op {
            // Init and Destroy handled on main thread only
            ll::Operation::Init(_) | ll::Operation::Destroy(_) => {
                warn!("Worker received Init/Destroy; ignoring");
                return Err(Errno::EIO);
            }
            ...
            ll::Operation::RemoveXAttr(x) => {
                se_fs.removexattr(self, self.request.nodeid().into(), x.name(), self.reply());
            }
            ll::Operation::Access(x) => {
            ...
            #[cfg(feature = "abi-7-16")]
            ll::Operation::BatchForget(x) => {
                se_fs.batch_forget(self, x.nodes());
            }
            #[cfg(feature = "abi-7-19")]
            ll::Operation::FAllocate(x) => {
                se_fs.fallocate(
            ...
            #[cfg(target_os = "macos")]
            ll::Operation::SetVolName(x) => {
                se_fs.setvolname(self, x.name(), self.reply());
            }
            #[cfg(target_os = "macos")]
            ll::Operation::GetXTimes(_) => {
                se_fs.getxtimes(self, self.request.nodeid().into(), self.reply());
            }
            #[cfg(target_os = "macos")]
            ll::Operation::Exchange(x) => {
                se_fs.exchange(
