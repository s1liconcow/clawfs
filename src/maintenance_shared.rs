...
#[macro_export]
macro_rules! clawfs_define_maintenance_module {
    () => {
        ...
        pub async fn create_checkpoint(
            ...
            let payload =
                encode_checkpoint_bytes(&superblock, created_at_unix, Some("hosted maintenance"));
        ...
        pub async fn enforce_retention_policy(
            ...
            let cutoff_nanos = if policy.max_age_days == 0 {
                ...
                let cutoff =
                    OffsetDateTime::now_utc() - time::Duration::days(policy.max_age_days as i64);
        ...
        pub async fn cleanup_expired_prefix(
            ...
            let cutoff = (OffsetDateTime::now_utc()
                - time::Duration::days(policy.expiry_days as i64))
        ...
        async fn compact_segment_batch(
            ...
            for (mut record, old_ptrs, _) in dataset {
                ...
                segments_to_delete.extend(old_ptrs.into_iter().map(|ptr| (ptr.generation, ptr.segment_id)));
            }
        ...
        #[cfg(test)]
        mod maintenance_shared_tests {
            use std::time::Duration;

            use super::{
                CheckpointConfig, CompactionConfig, LifecyclePolicy, checkpoint_object_key,
